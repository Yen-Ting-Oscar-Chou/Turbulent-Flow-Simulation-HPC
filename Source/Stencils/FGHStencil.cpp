...
void Stencils::FGHStencil::apply(const Parameters& parameters, FlowField& flowField, int i, int j) {
  ...
  values[0] = computeF2D(localVelocity_, localMeshsize_, parameters, parameters.timestep.dt);
  values[1] = computeG2D(localVelocity_, localMeshsize_, parameters, parameters.timestep.dt);
}
...
void Stencils::FGHStencil::apply(const Parameters& parameters, FlowField& flowField, int i, int j, int k) {
  ...
  if ((obstacle & OBSTACLE_SELF) == 0) { // If the cell is fluid
    ...
    if ((obstacle & OBSTACLE_RIGHT) == 0) { // If the right cell is fluid
      values[0] = computeF3D(localVelocity_, localMeshsize_, parameters, parameters.timestep.dt);
    }
    if ((obstacle & OBSTACLE_TOP) == 0) {
      values[1] = computeG3D(localVelocity_, localMeshsize_, parameters, parameters.timestep.dt);
    }
    if ((obstacle & OBSTACLE_BACK) == 0) {
      values[2] = computeH3D(localVelocity_, localMeshsize_, parameters, parameters.timestep.dt);
    }
  }
}
