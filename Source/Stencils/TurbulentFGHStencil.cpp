...
void Stencils::TurbulentFGHStencil::apply(TurbulentFlowField& flowField, int i, int j, int k) {
  ...
  if ((obstacle & OBSTACLE_SELF) == 0) { // If the cell is fluid
    ...
    if ((obstacle & OBSTACLE_RIGHT) == 0) { // If the right cell is fluid
      values[0] = computeTurbulentF3D(localVelocity_, localViscosity_, localMeshsize_, parameters_, parameters_.timestep.dt);
    }
    if ((obstacle & OBSTACLE_TOP) == 0) {
      values[1] = computeTurbulentG3D(localVelocity_, localViscosity_, localMeshsize_, parameters_, parameters_.timestep.dt);
    }
    if ((obstacle & OBSTACLE_BACK) == 0) {
      values[2] = computeTurbulentH3D(localVelocity_, localViscosity_, localMeshsize_, parameters_, parameters_.timestep.dt);
    }
  }
}
