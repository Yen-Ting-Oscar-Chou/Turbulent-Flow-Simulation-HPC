...
void Stencils::VelocityStencil::applyGPU(const Parameters& parameters, VectorField& velocity, VectorField& fgh, 
                                        ScalarField& pressure, IntScalarField& flags, int i, int j) {
  const RealType dt       = parameters.timestep.dt;
  ...
  if ((obstacle & OBSTACLE_SELF) == 0) {    // If this is a fluid cell
    if ((obstacle & OBSTACLE_RIGHT) == 0) { // Check whether the neighbor is also fluid
      ...
      // yield second-order accuracy for uniform meshsizes.
      const RealType dx = 0.5 * (parameters.meshsize.getDx(i, j) + parameters.meshsize.getDx(i + 1, j));
      velocity.getVectorElement(i, j, 0)
        = fgh.getVectorElement(i, j, 0)
          - dt / dx * (pressure.getScalar(i + 1, j) - pressure.getScalar(i, j));
    } else { // Otherwise, set to zero.
    ...
    if ((obstacle & OBSTACLE_TOP) == 0) {
      const RealType dy = 0.5 * (parameters.meshsize.getDy(i, j) + parameters.meshsize.getDy(i, j + 1));
      velocity.getVectorElement(i, j, 1)
        = fgh.getVectorElement(i, j, 1)
          - dt / dy * (pressure.getScalar(i, j + 1) - pressure.getScalar(i, j));
    } else {
...
void Stencils::VelocityStencil::applyGPU(const Parameters& parameters, VectorField& velocity, VectorField& fgh, ScalarField& pressure, 
              IntScalarField& flags, int i, int j, int k) {
  const RealType dt       = parameters.timestep.dt;
  ...
  if ((obstacle & OBSTACLE_SELF) == 0) {
    if ((obstacle & OBSTACLE_RIGHT) == 0) {
      const RealType dx = 0.5 * (parameters.meshsize.getDx(i, j, k) + parameters.meshsize.getDx(i + 1, j, k));
      velocity.getVectorElement(i, j, k, 0)
        = fgh.getVectorElement(i, j, k, 0)
          - dt / dx * (pressure.getScalar(i + 1, j, k) - pressure.getScalar(i, j, k));
    } else {
      velocity.getVectorElement(i, j, k, 0) = 0.0;
    }
    if ((obstacle & OBSTACLE_TOP) == 0) {
      const RealType dy = 0.5 * (parameters.meshsize.getDy(i, j, k) + parameters.meshsize.getDy(i, j + 1, k));
      velocity.getVectorElement(i, j, k, 1)
        = fgh.getVectorElement(i, j, k, 1)
          - dt / dy * (pressure.getScalar(i, j + 1, k) - pressure.getScalar(i, j, k));
    } else {
      velocity.getVectorElement(i, j, k, 1) = 0.0;
    }
    if ((obstacle & OBSTACLE_BACK) == 0) {
      const RealType dz = 0.5 * (parameters.meshsize.getDz(i, j, k) + parameters.meshsize.getDz(i, j, k + 1));
      velocity.getVectorElement(i, j, k, 2)
        = fgh.getVectorElement(i, j, k, 2)
          - dt / dz * (pressure.getScalar(i, j, k + 1) - pressure.getScalar(i, j, k));
    } else {
