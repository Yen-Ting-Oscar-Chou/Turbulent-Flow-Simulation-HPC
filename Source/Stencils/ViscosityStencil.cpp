...
void Stencils::ViscosityStencil::applyGPU(const Parameters& parameters, VectorField& velocity, ScalarField& viscosity, ScalarField& distance, IntScalarField& flags, int i, int j) {
  ...
  const RealType dist = distance.getScalar(i, j);
  ...
  const RealType       deltaMixLen    = (turbulenceType == ZERO) * deltaZero(x, re) + (turbulenceType == LAMINAR) * deltaLaminar(x, re)
                               + (turbulenceType == TURBULENT) * deltaTurbulent(x, re);
  const RealType mixingLength = std::min(KAPPA * dist, 0.09 * deltaMixLen);
...
void Stencils::ViscosityStencil::applyGPU(
  const Parameters& parameters, VectorField& velocity, ScalarField& viscosity, ScalarField& distance, IntScalarField& flags, int i, int j, int k
) {
  ...
  const RealType dist = distance.getScalar(i, j, k);
